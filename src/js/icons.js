// Professional Lucide-inspired SVG icon system for omukun Shift

export function getIconSVG(name, { size = 18, className = '', ariaLabel = '' } = {}) {
  const ariaAttr = ariaLabel ? `aria-label="${ariaLabel}" role="img"` : 'aria-hidden="true"';
  const sizeAttr = `width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"`;
  const classAttr = className ? `class="icon icon-${name} ${className}"` : `class="icon icon-${name}"`;
  ...
  const pathHTML = paths[name] || paths['calendar'];
  return `<svg ${sizeAttr} ${classAttr} ${ariaAttr}>${pathHTML}</svg>`;
}
